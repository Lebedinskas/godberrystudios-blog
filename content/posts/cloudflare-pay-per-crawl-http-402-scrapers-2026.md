...
When I shipped the Yelp Scraper, the move that actually worked against DataDome was the same move that survives a `402` future: extract structured data (JSON-LD `@type:Restaurant` blocks) instead of LLM-parsing the page. One fetch, one parse, fully deterministic — and on a metered future you only pay for the one fetch. The "grab the whole page, let the LLM sort it out" pattern is the most expensive thing you can do per record under both regimes, and the gap is going to widen. Getting past the gate is only half the job, though — [a clean fetch can still hand you decoy data](/posts/scraper-data-verification-cloudflare-decoy-200-ok-2026/) instead of the real page.
