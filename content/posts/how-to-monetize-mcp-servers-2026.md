...
The [WebMCP standard going into Chrome](/posts/webmcp-chrome-ai-agents-explained/) only accelerates this — when every website exposes an MCP surface to the user's local agent, pricing clarity becomes the difference between getting called and getting skipped. AWS Bedrock AgentCore Payments shipped in preview on 2026-05-07 and made all of this concrete at production scale; for the seller-side decision tree on adding x402 to an MCP server, an Apify actor, or a small API product, see the [AgentCore Payments operator playbook](/posts/aws-bedrock-agentcore-payments-operator-playbook-2026/). The same structural rule applies on marketplaces with usage-based pricing — public-facing usage dashboards and seller-side revenue dashboards are different screens. The [Apify MAU vs Insights field note](/posts/apify-mau-decoy-vs-insights-revenue-2026/) is the operator-side version for one of those marketplaces.
